...
// Ring is an interface to interact with one or more rings.
type Ring interface {
	CQ() *CompletionQueue
	EventFd() int
	...
	Fd() int
	SQ() *SubmitQueue
	FileReadWriter(f *os.File) (ReadWriteSeekerCloser, error)
	SubmitEntry() (*SubmitEntry, func())
	SockoptListener(network, address string, errHandler func(error), sockopts ...int) (net.Listener, error)
	Stop() error
}
